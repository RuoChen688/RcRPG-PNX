...
public class Guild {
    ...
    public static void addGuild(Player player, String guild){
        RcRPGMain.instance.saveResource("Guild.yml","/Guild/"+guild+".yml",false);
        Config config = new Config(RcRPGMain.instance.getGuildFile()+"/"+guild+".yml");
        config.set("名称",guild);
        config.set("公会资金", RcRPGMain.instance.config.getInt("公会创建初始资金"));
        config.set("公会人数", RcRPGMain.instance.config.getInt("公会初始人数"));
        config.save();
    ...
    public static Config getGuildConfig(String guild){
        return new Config(RcRPGMain.instance.getGuildFile()+"/"+guild+".yml");
    }
    ...
    public static void dismissGuild(Player player){
        ...
        for(String name : Guild.getAllMember(player)){
            ...
            pConfig.set("公会", RcRPGMain.instance.config.get("初始公会"));
            pConfig.save();
        }
        File file = new File(RcRPGMain.instance.getGuildFile(),"/"+guild+".yml");
        file.delete();
    }
    ...
    public static void kickGuild(Player player,String name){
        ...
        config.set("公会", RcRPGMain.instance.config.get("初始公会"));
        config.save();
        config = new Config(RcRPGMain.instance.getGuildFile()+"/"+guild+".yml");
    ...
    public static int getUpdateMoney(Player player){
        String guild = Guild.getGuild(player);
        Config config = new Config(RcRPGMain.instance.getGuildFile()+"/"+guild+".yml");
        ArrayList<String> list = (ArrayList<String>) RcRPGMain.instance.config.getStringList("公会升级金币");
        for(String s : list){
            String[] arr = s.split(":");
    ...
    public static int getUpdateSize(Player player){
        String guild = Guild.getGuild(player);
        Config config = new Config(RcRPGMain.instance.getGuildFile()+"/"+guild+".yml");
        ArrayList<String> list = (ArrayList<String>) RcRPGMain.instance.config.getStringList("公会升级人数");
        for(String s : list){
            String[] arr = s.split(":");
